...
public class WordGui extends JFrame {
	...
	public void setGui() {
		...
		// add action from handler
		itemEdit.addActionListener(eh);
		EditMenu.add(itemEdit);

		EditMenu.addSeparator(); // add a horizontal separator line

		itemEdit = new JMenuItem("Clear");
		itemEdit.addActionListener(eh);
		EditMenu.add(itemEdit);

		itemEdit = new JMenuItem("Find");
	...
	public static void taskWordGui(TreeMap<Word, Word> map) {
		...
		while (i.hasNext()) {
			...
			Word key = me.getValue(); // get the value
			// convert word to string
			String value = key + ""; 

			if (wordCheck(value)) { // Print out only word that fully letter
